...
- `sceneSerializer` 提供 DTO 读写能力，并保留 `hydrateScene` / `deserializeScene` 兼容运行时 class 实例。
...
- `src/engine/scene/sceneOperations.ts`
  - 提供纯函数：新增/删除/复制实体、图层移动、组件字段更新、场景文件夹操作等。
- `src/engine/serialization/sceneSerializer.ts`
  - DTO 序列化入口：`serializeSceneData`、`deserializeSceneData`。
  - 兼容 class 边界：`sceneToData`、`hydrateScene`、`serializeScene`、`deserializeScene`。
- `src/stores/scene.ts`
  - 轻量 Pinia store。
- `src/stores/sceneActions.ts`
  - 暂存现有编辑器集成 action，后续会继续拆分到更小的 domain/service 模块。

## 构建产物策略

`dist/` 与 `dist-electron/` 是构建产物，不应入库。它们由以下命令生成：
...
1. 让 `scene.ts` 的 state 直接保存 `SceneData[]` 与 `currentSceneData`。
2. Renderer/Runtime 入口按需调用 `hydrateScene(sceneData)`。
3. Inspector/SceneTree 等编辑面板改为调用 `SceneOperations` 修改 DTO。
4. 删除对 class 实例字段公开化的临时兼容需求。
5. 将 `sceneActions.ts` 继续拆成 `scenePersistence`、`prefabActions`、`folderActions`、`entityFactory` 等模块。
