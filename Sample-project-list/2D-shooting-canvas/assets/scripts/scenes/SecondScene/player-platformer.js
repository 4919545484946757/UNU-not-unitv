...
const CONSUMABLES = {
  [itemId('bandage')]: { displayName: 'Bandage', heal: 20 },
  [itemId('medkit')]: { displayName: 'Medkit', heal: 55 },
  [itemId('scp_500')]: { displayName: 'SCP-500', heal: 'full' }
}
...
const heldSlotIndex = (ctx) => 17 + selectedHotbar(ctx)

const inventoryPanel = (ctx) => ctx.scene.getEntityById('ui_inventory_panel') || ctx.api.findEntityByName('InventoryPanel_HTML')

const syncInventoryPanel = (ctx, entity) => {
  const state = ctx.api.getState(entity)
  if (!Array.isArray(state.inventoryItems)) state.inventoryItems = defaultInventory()
  state.inventoryItems = normalizeInventory(state.inventoryItems)
  const panel = inventoryPanel(ctx)
  if (!panel) return
  const panelState = ctx.api.getState(panel)
  panelState.items = state.inventoryItems.slice()
  panelState.selectedHotbar = clampHotbar(state.selectedHotbar ?? panelState.selectedHotbar ?? 1)
}
...
const ensureHealth = (ctx, entity) => {
  const state = ctx.api.getState(entity)
  const data = healthData(entity)
  const maxHealth = Math.max(1, Number(data?.max ?? state.maxHealth ?? 100))
  state.maxHealth = Number.isFinite(Number(state.maxHealth)) ? Math.max(1, Number(state.maxHealth)) : maxHealth
  state.health = Number.isFinite(Number(state.health)) ? Number(state.health) : Number(data?.current ?? state.maxHealth)
...
const useInventoryItem = (ctx, entity, slotIndex) => {
  ...
  if (!Array.isArray(state.inventoryItems)) state.inventoryItems = defaultInventory()
  state.inventoryItems = normalizeInventory(state.inventoryItems)
  const index = Math.max(0, Math.min(state.inventoryItems.length - 1, Number(slotIndex) || 0))
  const item = String(state.inventoryItems[index] || '').trim()
  const consumable = CONSUMABLES[item]
  if (!consumable) return false
  const before = ensureHealth(ctx, entity).health
  const next = consumable.heal === 'full'
    ? Number(ctx.api.getState(entity).maxHealth)
    : Number(before) + Number(consumable.heal || 0)
  const health = setHealth(ctx, entity, next)
  state.inventoryItems[index] = ''
  syncInventoryPanel(ctx, entity)
  updateHeldHud(ctx, entity)
  ctx.api.log(`[Inventory] used ${consumable.displayName}: HP ${Math.round(before)} -> ${Math.round(health.health)}`)
  return true
}

const updateHeldHud = (ctx, entity) => {
  const hud = ctx.scene.getEntityById('ui_held_item_hud') || ctx.api.findEntityByName('HeldItemHUD')
  const ui = hud?.getComponent('UI')
  if (!ui) return
  const health = ensureHealth(ctx, entity)
  const state = ctx.api.getState(entity)
  if (!Array.isArray(state.inventoryItems)) state.inventoryItems = defaultInventory()
  state.inventoryItems = normalizeInventory(state.inventoryItems)
  const item = state.inventoryItems[heldSlotIndex(ctx)] || ''
  const name = WEAPONS[item]?.displayName || CONSUMABLES[item]?.displayName || item || 'Empty'
  ui.text = `HP: ${Math.round(health.health)}/${Math.round(health.maxHealth)}\nHeld: ${name}`
}
...
export default {
  onUpdate(ctx) {
    ...
    if (ctx.api.input.wasActionPressed('use_item')) {
      useInventoryItem(ctx, ctx.entity, heldSlotIndex(ctx))
    }
    fireHeldWeapon(ctx, ctx.entity)
    updateHeldHud(ctx, ctx.entity)
  }
}
